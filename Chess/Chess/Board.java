public class Board {
    ...
    public Piece getPiece(int inputPosition){
        // Returns the piece at a certain positionn
        int[] Pos = convertPos(inputPosition);
        return this.Board[Pos[0]][Pos[1]];
    }

    public int[] convertPos(int inputPosition){
        // Converts the integer position to a 2 digit array
    ...
    public Boolean isEmpty(int inputPosition){
        // Returns if a given integer position is empty
        int[] Pos = convertPos(inputPosition);
        ...
        for (Piece[] row: this.Board){
            ...
            for (Piece column: row){
                if (isEmpty(RowNum * 10 + ColumnNum)){
                    ...
                    if (!piece.getColor()){
                        // If the piece is black, make it lowercase, otherwise uppercase
